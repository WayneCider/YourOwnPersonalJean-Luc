    ...
    while True:
        ...
        if user_input.startswith("/"):
            if user_input.strip().lower() == "/retry":
                ...
                result = _handle_command(user_input, registry, context, learner, undo_stack, audit=audit, config=config)
                if result == "exit":
                    break
                if result == "continue":
                    continue
...
def _handle_command(cmd: str, registry, context=None, learner=None, undo_stack=None, audit=None, config=None) -> str:
    ...
    if cmd_name == "/exit":
        _show_exit_summary(learner, audit=audit)
        print(f"{_DIM}Goodbye.{_RESET}")
        return "exit"
